...
export async function POST(request: NextRequest) {
  try {
    ...
    // Create an admin client to bypass RLS constraints
    ...
    try {
      // First, check if the user folder exists by listing it using admin client
      const { data: folderExists, error: folderError } = await supabaseAdmin
        .storage
        .from('antique-images')
      ...
      // Upload processed image to Supabase Storage using admin client to bypass RLS
